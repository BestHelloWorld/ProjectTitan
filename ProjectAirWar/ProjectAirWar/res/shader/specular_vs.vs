...
void main()
{
	...
	if(diffuse.x > 0.0)
	{
		vec4 viewDir = V_CameraPos - worldPos;
		viewDir = normalize(viewDir);
		vec4 reflectLight = vec4(reflect(-L, N), 1.0);
		specular = V_SpecularMaterial*V_SpecularLight*pow(max(0.0, dot(viewDir, reflectLight)), 4.0);
	}
