...
void Aircraft::Move(FLOAT x, FLOAT y, FLOAT z)
{
	...
	z = abs(x) > 0.0f ? abs(x) : abs(z);

	if (x > 0)
	{
		x *= mRollAngle / AIRCRAFT_MAX_ROLL_ANGLE + 0.5f;
	}
	else if (x < 0)
	{
		x *= -mRollAngle / AIRCRAFT_MAX_ROLL_ANGLE + 0.5f;
...
void Aircraft::ResetPosition()
{
	*mPosition = cm::vec3(0.0f);
}
