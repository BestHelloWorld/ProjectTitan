...
void CheckError(const char*file, const int line)
{
	...
	if (error != GL_NO_ERROR)
	{
		...
		case GL_INVALID_ENUM:
			LOG_D("error:GL_INVALID_ENUM %s : %d\n", file, line);
			break;
...
UINT CreateTexture2DFromBMP(const CHAR* path)
{
    ...
    if(fileContent == 0) return NEGATIVE_ONE;

    INT imgWidth, imgHeight;
    UCHAR* imgData = DecodeBMP(fileContent, imgWidth, imgHeight);
    if(imgData == 0) return NEGATIVE_ONE;
...
UINT CreateTextureCubeFromBMP(const CHAR* frontPath, const CHAR* backPath, const CHAR* leftPath, const CHAR* rightPath, const CHAR* topPath, const CHAR* bottomPath)
{
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, frontImgData);
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, bottomImgData);
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, leftImgData);
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, backImgData);
    ...
    glTexImage2D( GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, topImgData);
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, rightImgData);
