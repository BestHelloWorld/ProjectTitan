...
vec4 CalcHBlur()
{
	...
	{
		color += texture2D(U_Texture, vec2(V_Texcoord.x + (offset*i*pow(-1, i)),V_Texcoord.y)) * weight[i];
	}
	return color;
}
...
vec4 CalcVBlur()
{
	...
	for(int i = 0; i < weight.length; ++i)
	{
		color += texture2D(U_Texture, vec2(V_Texcoord.x,V_Texcoord.y + (offset*i*pow(-1, i)))) * weight[i];
	}
	return color;
}
